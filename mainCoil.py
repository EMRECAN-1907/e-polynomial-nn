"""
mainCOIL.py — COIL-20 Nesne Tanıma
Normal NN vs e-Poly NN
Giriş : 1024 özellik (32x32 piksel nesne fotoğrafı)
Çıkış : 20 sınıf (20 farklı nesne)
e-Poly: group_size=8 → 1024/8 = 128 giriş
"""
...
# ── VERİ ─────────────────────────────────────────────────────────────────────
print("COIL-20 verisi yükleniyor...")
_data = fetch_openml('coil-20', version=1, as_frame=False, parser='liac-arff')
...
X_comp = e_compress(X_ham, group_size=8)
...
def egit(X_tr, y_tr, X_te, y_te, dims, epochs, lr=0.01, bs=32, label=""):
...
# ── ÇALIŞTIR ──────────────────────────────────────────────────────────────────
p_n, t_n, par_n, L_n = egit(X_n_tr, y_tr, X_n_te, y_te, [1024, 256, 128, N_SINIF], 200, label="Normal NN")
p_c, t_c, par_c, L_c = egit(X_c_tr, y_tr, X_c_te, y_te, [128,  128,  64, N_SINIF], 300, label="e-Poly NN")

# ── SONUÇLAR ──────────────────────────────────────────────────────────────────
n_giriş_n, n_giriş_c = X_n_tr.shape[1], X_c_tr.shape[1]
print("\n" + "═"*56)
print(f"  {'Metrik':<22} {'Normal NN':>14} {'e-Poly NN':>14}")
print("─"*56)
print(f"  {'Giriş nöronu':<22} {n_giriş_n:>14} {n_giriş_c:>14}")
