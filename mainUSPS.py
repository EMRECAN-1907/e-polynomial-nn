"""
mainUSPS.py — USPS Posta Kodu Rakam Tanıma
Normal NN vs e-Poly NN
Giriş : 256 özellik (16x16 piksel)
Çıkış : 10 sınıf (0-9)
e-Poly: group_size=8 → 256/8 = 32 giriş
"""
...
# ── VERİ ─────────────────────────────────────────────────────────────────────
print("USPS verisi yükleniyor...")
_data = fetch_openml('usps', version=2, as_frame=False, parser='liac-arff')
...
X_comp = e_compress(X_ham, group_size=4)  # 256 → 32
...
def egit(X_tr, y_tr, X_te, y_te, dims, epochs, lr=0.01, bs=64, label=""):
...
# ── ÇALIŞTIR ──────────────────────────────────────────────────────────────────
p_n, t_n, par_n, L_n = egit(X_n_tr, y_tr, X_n_te, y_te, [256, 128, 64, N_SINIF], 200, label="Normal NN")
p_c, t_c, par_c, L_c = egit(X_c_tr, y_tr, X_c_te, y_te, [64,   64, 32, N_SINIF], 300, label="e-Poly NN")

# ── SONUÇLAR ──────────────────────────────────────────────────────────────────
print("\n" + "═"*56)
print(f"  {'Metrik':<22} {'Normal NN':>14} {'e-Poly NN':>14}")
print("─"*56)
print(f"  {'Giriş nöronu':<22} {256:>14} {64:>14}")
