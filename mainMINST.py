"""
mainMNIST.py — MNIST Karşılaştırma (Normal NN vs e-Poly NN)
Tek dosya, import yok.
"""
...
# ── e-POLİNOM ─────────────────────────────────────────────────────────────────
def matris_to_polinom(X, group_size=8):
    n, d = X.shape
...
_data = fetch_openml('mnist_784', version=1, as_frame=False, parser='liac-arff')
X_ham = _data.data.astype(float)
_le   = LabelEncoder()
y     = _le.fit_transform(_data.target)

X_comp = matris_to_polinom(X_ham, group_size=8)  # 784 → 98

X_tr_raw, X_te_raw, y_tr, y_te = train_test_split(X_ham,  y, test_size=0.2, random_state=42, stratify=y)
X_c_tr,   X_c_te,   _,    _    = train_test_split(X_comp, y, test_size=0.2, random_state=42, stratify=y)

sx_n = MinMaxScaler(); X_n_tr = sx_n.fit_transform(X_tr_raw); X_n_te = sx_n.transform(X_te_raw)
...
# ── ÇALIŞTIR ──────────────────────────────────────────────────────────────────
p_n, t_n, par_n, L_n = egit(X_n_tr, y_tr, X_n_te, y_te, [784, 128, 64, N_SINIF], 200, label="Normal NN")
p_c, t_c, par_c, L_c = egit(X_c_tr, y_tr, X_c_te, y_te, [98,   64, 32, N_SINIF], 300, label="e-Poly NN")

# ── SONUÇLAR ──────────────────────────────────────────────────────────────────
print("\n" + "═"*56)
print(f"  {'Metrik':<22} {'Normal NN':>14} {'e-Poly NN':>14}")
print("─"*56)
print(f"  {'Giriş nöronu':<22} {784:>14} {98:>14}")
